    ...
    mainControllers.controller(
        'SearchCtrl', [
            ...
            function SearchCtrl($scope, Tag, Search, User) {
                $scope.search = {};
                $scope.search.opportunityTypes = ['mentorship', 'funding',
                    'employment',
                    'networking'
                ];
                // tags to be searched
                ...
                $scope.runSearch = function () {
                    $scope.search.results = Search.search({
                        ...
                        opportunityTypes: $scope.search.opportunityTypes.filter(
                            function (
                                type) {
                                return type != '';
                            }).join(',')
                ...
                $scope.tagComplete = function () {
                    ...
                    promise.then(function (tagSuggestions) {
                        $scope.search.tagSuggestions = tagSuggestions.filter(
                            function (tag) {
